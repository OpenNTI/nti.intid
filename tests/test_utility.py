...
class TestUtility(AbstractTestBase):
	...
	def test_ops(self):
		...
		assert_that(eventtesting.getEvents(IIdAddedEvent), has_length(1))
		...
		assert_that(calling(u.getId).with_args(obj), raises(KeyError))
		assert_that(eventtesting.getEvents(IIdRemovedEvent), has_length(1))
	...
	def test_no_event(self):
		...
		u.register(obj, event=False)
		assert_that(eventtesting.getEvents(IIdAddedEvent), has_length(0))

		u.unregister(obj, event=False)
		assert_that(eventtesting.getEvents(IIdRemovedEvent), has_length(0))
